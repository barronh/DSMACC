...
if 'R4N2' in data.columns:
    ...
    NOYSPCS = 'NOx NXOYN HNO2 HNO3 PANS HNO4 NTR'.split()
else:
    ...
    NOYSPCS = 'NOx NXOYN HONO HNO3 PANS HO2NO2 NTR NOA NA'.split()

data.eval('N2O5N = 2*N2O5', inplace=True)
data.eval('NXOYN = N2O5N + NO3', inplace=True)
pans = [
    k for k in data.columns if 'PAN' in k
] + 'PPN PHAN PBENZ IPBENZ'.split(' ')
...
if haspbl and hastemp:
    ...
    anchor = (.6, 1.05)
pax = data.plot(
    x='JDAY', y=['O3', 'C5H8'], linestyle='-', legend=False, color=['k', 'g']
)
...
data.plot(
    x='JDAY', y=NOYSPCS, secondary_y=False, ax=sax, stacked=True, legend=False,
    linestyle='--'
)
...
if haspbl:
    ...
    data.plot(
        x='JDAY', y=['PBL'], linestyle='-', legend=False, ax=pblax, color='b'
    )
...
if hastemp:
    ...
    data.plot(
        x='JDAY', y=['TEMP'], linestyle='-', legend=False, ax=tempax, color='r'
    )
...
sax.set_ylabel('$NOy_i$')
pax.legend(
    patches, labels, loc='lower center', bbox_to_anchor=anchor, ncol=5,
    handlelength=None
)
